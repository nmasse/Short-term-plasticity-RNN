...
class RNNCellSTP(RNNCell):
    ...
    def call(self, inputs, state, scope=None):
        ...
        with tf.variable_scope('rnn_cell'):
            W_in = tf.get_variable('W_in', initializer = self.w_in0, trainable=True)
            W_rnn = tf.get_variable('W_rnn', initializer = self.w_rnn0, trainable=True)
            b_rnn = tf.get_variable('b_rnn', initializer = self.b_rnn0, trainable=True)
            W_ei = tf.get_variable('EI', initializer = self.EI_matrix, trainable=False)
