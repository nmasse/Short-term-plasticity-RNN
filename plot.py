...
def plot(filename,neuronal,synaptic,stability,syn_stability):
    ...
    plt.savefig('./savedir/plots2/'+filename[:-4]+'.png')
    plt.close()

    plt.figure()
    plt.imshow(np.mean(stability[0,0],axis=0))
    plt.show('./savedir/plots2/'+filename[:-4]+'_heatmap_neuronal.png')
    plt.close()
    quit()

    plt.figure()
    plt.imshow(np.mean(syn_stability[0,0],axis=0))
    plt.savefig('./savedir/plots2/'+filename[:-4]+'_heatmap_synaptic.png')
    plt.close()


if __name__ == "__main__":
    files = os.listdir('./savedir/output2')
    files2 = os.listdir('./savedir/test2')
    ...
    for filename in files[::-1]:
        analysis_new.analyze_model_from_file('./savedir/output2/'+filename)
